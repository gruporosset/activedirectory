...
var ActiveDirectory = function (url, baseDN, username, password, defaults) {
  if (this instanceof ActiveDirectory) {
    ...
    defaultAttributes = _.extend(
      {},
      originalDefaultAttributes,
      (this.opts || {}).attributes || {},
      (defaults || {}).attributes || {}
    );
    defaultReferrals = _.extend(
      {},
 ...
 */
async function createClient(url, opts) {
  ...
  log.debug(
    ...
    opts.url,
    _.omit(opts, "url", "bindDN", "bindCredentials")
  );
  ...
  return await promise.then((res) => res).catch((err) => null);
}
...
function search(baseDN, opts, callback) {
  ...
  var entryParser =
    (opts || {}).entryParser ||
    (self.opts || {}).entryParser ||
    function onEntryParser(item, raw, callback) {
      callback(item);
    };
  ...
  if (
    !_.any(controls, function (control) {
    ...
    controls.push(
      new ldap.PagedResultsControl({ value: { size: defaultPageSize } })
    );
  }
  if (opts.includeDeleted) {
    if (
      ...
      controls.push(
        new ldap.Control({ type: "1.2.840.113556.1.4.417", criticality: true })
      );
    }
  }
...
function parseRangeAttributes(result, opts, callback) {
  ...
  var rangeAttributes =
    RangeRetrievalSpecifierAttribute.prototype.getRangeAttributes(result);
  ...
  _.each(rangeAttributes, function (rangeAttribute, index) {
    ...
    Array.prototype.push.apply(
      result[rangeAttribute.attributeName],
      result[rangeAttribute.toString()]
    );
...
function getGroupMembershipForDN(opts, dn, stack, callback) {
  ...
  log.trace(
    "getGroupMembershipForDN(%j,%s,stack:%j)",
    opts,
    dn,
    (stack || []).length
  );
  ...
  if (!dn) {
    var error = new Error(
      "No distinguishedName (dn) specified for group membership retrieval."
    );
    log.error(error);
  ...
  opts = _.defaults(_.omit(opts || {}, "filter", "scope", "attributes"), {
    ...
    attributes: joinAttributes(
      (opts || {}).attributes || defaultAttributes.group,
      ["groupType"]
    ),
  });
    ...
    async.forEach(
      ...
      function (group, asyncCallback) {
        ...
        if (isGroupResult(group)) {
          ...
          log.debug(
            'Retrieving nested group membership for group "%s"',
            group.dn
          );
          getGroupMembershipForDN.call(
            self,
            opts,
            group.dn,
            groups,
            function (err, nestedGroups) {
              if (err) {
                asyncCallback(err);
                return;
              }
              ...
              log.debug(
                'Group "%s" which is a member of group "%s" has %d nested group(s). Nested: %j',
                group.dn,
                dn,
                nestedGroups.length,
                _.map(nestedGroups, function (group) {
                  return group.dn;
                })
              );
      ...
      function (err) {
        ...
        groups = _.uniq(
          _.sortBy(groups, function (group) {
...
function getUserDistinguishedName(opts, username, callback) {
  ...
  if (isDistinguishedName.call(self, username)) {
    log.debug(
      '"%s" is already a distinguishedName. NOT performing query.',
      username
    );
 ...
 * @private
 */
ActiveDirectory.prototype._getDefaultAttributes =
 ...
 * @private
 */
ActiveDirectory.prototype._getDefaultUserAttributes =
 ...
 * @private
 */
ActiveDirectory.prototype._getDefaultGroupAttributes =
 ...
 */
ActiveDirectory.prototype.getUsersForGroup = function getUsersForGroup(
  opts,
  groupName,
  callback
) {
  ...
  self.findGroup(
    _.defaults({}, _.omit(opts || {}, "attributes"), {
      attributes: joinAttributes(
        (opts || {}).attributes || defaultAttributes.group,
        ["member"]
      ),
    ...
    function (err, group) {
      ...
      async.each(
        chunks,
        function getUsersForGroup_ChunkItem(members, asyncChunkCallback) {
          ...
          var filter = _.reduce(
            members || [],
            function (memo, member, index) {
              return (
                memo +
                "(distinguishedName=" +
                parseDistinguishedName(member) +
                ")"
              );
            },
            ""
          ...
          var localOpts = {
            ...
            scope: "sub",
            attributes: joinAttributes(
              (opts || {}).attributes || defaultAttributes.user || [],
              getRequiredLdapAttributesForUser(opts),
              ["groupType"]
            ),
          };
...
*/
          log.info(
            '%d user(s) belong in the group "%s"',
            users.length,
            groupName
          );
 ...
 * @param {Function} [callback] The callback to execute when completed. callback(err: {Object}, groups: {Array[Group]})
 */
ActiveDirectory.prototype.getGroupMembershipForUser =
  function getGroupMembershipForUser(opts, username, callback) {
    ...
    getUserDistinguishedName.call(self, opts, username, function (err, dn) {
      ...
      if (!dn) {
        log.warn(
          'Could not find a distinguishedName for the specified username: "%s"',
          username
        );
 ...
 * @param {Function} [callback] The callback to execute when completed. callback(err: {Object}, groups: {Array[Group]})
 */
ActiveDirectory.prototype.getGroupMembershipForGroup =
  function getGroupMembershipForGroup(opts, groupName, callback) {
    ...
    getGroupDistinguishedName.call(self, opts, groupName, function (err, dn) {
      ...
      if (!dn) {
        log.warn(
          'Could not find a distinguishedName for the specified group name: "%s"',
          groupName
        );
        if (callback) callback();
        return;
 ...
 */
ActiveDirectory.prototype.userExists = function userExists(
  opts,
  username,
  callback
) {
 ...
 * @param {Function} callback The callback to execute when completed. callback(err: {Object}, result: {Boolean})
 */
ActiveDirectory.prototype.groupExists = function groupExists(
  opts,
  groupName,
  callback
) {
 ...
 * @param {Function} callback The callback to execute when completed. callback(err: {Object}, result: {Boolean})
 */
ActiveDirectory.prototype.isUserMemberOf = function isUserMemberOf(
  opts,
  username,
  groupName,
  callback
  ...
  self.getGroupMembershipForUser(opts, username, function (err, groups) {
    ...
    if (!groups || groups.length === 0) {
      log.info(
        '"%s" IS NOT a member of "%s". No groups found for user.',
        username,
        groupName
      );
      callback(null, false);
    ...
    var result = _.any(groups, function (item) {
      return (
        (item.dn || "").toLowerCase() === lowerCaseGroupName ||
...
ActiveDirectory.prototype.find = function find(opts, callback) {
  ...
  var localOpts = _.defaults(_.omit(opts || {}, "attributes"), {
    ...
    attributes: joinAttributes(
      ...
      getRequiredLdapAttributesForGroup(opts),
      getRequiredLdapAttributesForUser(opts),
      ["objectCategory"]
    ),
  });
  search.call(self, localOpts, function onFind(err, results) {
    ...
    if (!results || results.length === 0) {
      log.warn(
        ...
        truncateLogOutput(localOpts.filter)
      );
      if (callback) callback();
      self.emit("done");
      return;
    ...
    async.forEach(
      ...
      function (item, asyncCallback) {
        if (isGroupResult(item)) {
          ...
          var other = pickAttributes(
            item,
            (opts || {}).attributes ||
              _.union(defaultAttributes.user, defaultAttributes.group)
          );
...
ActiveDirectory.prototype.findDeletedObjects = function find(opts, callback) {
  ...
  function searchDeletedObjects(baseDN, opts) {
    search.call(
      self,
      baseDN,
      _.defaults({}, opts, { includeDeleted: true }),
      function onFind(err, results) {
        ...
        if (!results || results.length === 0) {
          log.warn(
            'No deleted objects found for query "%s"',
            truncateLogOutput(opts.filter)
          );
          if (callback) callback();
          self.emit("done");
  ...
  if (!localOpts.baseDN) {
    log.debug(
      "No baseDN specified for Deleted Object. Querying RootDSE at %s.",
      self.opts.url
    );
    ActiveDirectory.prototype.getRootDSE(
      ...
      function (err, result) {
        ...
        log.info(
          "Retrieved defaultNamingContext (%s) from RootDSE at %s.",
          result.defaultNamingContext,
          self.opts.url
        );
        searchDeletedObjects(
          "CN=Deleted Objects," + result.defaultNamingContext,
          localOpts
        );
      }
    );
 ...
 */
ActiveDirectory.prototype.findGroup = function findGroup(
  opts,
  groupName,
  callback
) {
  ...
  search.call(self, localOpts, function onSearch(err, results) {
    ...
    log.info(
      '%d group(s) found for query "%s". Returning first group: %j',
      results.length,
      truncateLogOutput(localOpts.filter),
      group
    );
...
ActiveDirectory.prototype.findGroups = function findGroup(opts, callback) {
  ...
  search.call(self, localOpts, function onSearch(err, results) {
    ...
    if (!results || results.length === 0) {
      log.warn(
        'No groups found matching query "%s"',
        truncateLogOutput(localOpts.filter)
      );
      if (callback) callback();
      return;
 ...
 */
ActiveDirectory.prototype.findUser = function findUser(
  opts,
  username,
  includeMembership,
  callback
  ...
  search.call(self, localOpts, function onSearch(err, results) {
    ...
    log.info(
      '%d user(s) found for query "%s". Returning first user: %j',
      results.length,
      truncateLogOutput(localOpts.filter),
      user
    );
 ...
 */
ActiveDirectory.prototype.findUsers = function findUsers(
  opts,
  includeMembership,
  callback
) {
 ...
 */
ActiveDirectory.prototype.authenticate = function authenticate(
  username,
  password,
  callback
) {
 ...
 */
ActiveDirectory.prototype.getRootDSE = function getRootDSE(
  url,
  attributes,
  callback
) {
  ...
  if (!url)
    throw new Error(
      'No url specified for the root DSE. Please specify an ldap url in the following format: "ldap://yourdomain.com:389".'
    );
